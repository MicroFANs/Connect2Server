...
public class UploadActivity extends AppCompatActivity {
    ...
    private static final String ROUTE_UP3 = "/upOneSampleFromCandidate"; //route，对应flask里的route
...
//                }
    ...
    /* 阶段1 */
    //从用户项集中随机获取一个项并扰动
    private Integer sampleOne_perturbed(ArrayList<Record> list){
...
//        String json= msgObj.toString();
    ...
    //扰动交集的数目
    private void number_upload(OkHttpClient client,String route){
        ...
        double p=OLH.p_value(mEpsilon, hashDomain);
        int y=OLH.grr(p,x,hashDomain);
        addResponse(new Response("Step3：上传扰动交集长度成功!","扰动交集数目为："+ y,TimeUtil.getTime()));//增加记录
        //更新进度条
        progressIndex.add(2);
        pointProcessBar.refreshSelectedIndexSet(progressIndex);
        HashMap<String,String> map=new HashMap<>();//使用HashMap生成Json
    ...
    /*阶段3*/
    private ArrayList<Record> padSet(int length,ArrayList<Record> records){
        Random r=new Random();
        for (int i = 0; i <length ; i++) {
            records.add(new Record(5080+(i+1), null, 0, r.nextInt(2)-1));
        }
        return records;
    }

    private void OneSampleFromCandidate_upload(){}
